...
import { X, Plus, Package, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function AddProductModal({ products = [], onClose, onSuccess }) {
  ...
  const [storeQty, setStoreQty] = useState(10);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Successfully added item '${res.data.name}'!`);
      onSuccess && onSuccess();
      onClose();
    } catch (e) {
    ...
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 p-6 text-white flex items-center justify-between border-b border-amber-500/30 shrink-0">
          <div>
            <div className="lss-overline text-amber-300">Inventory Master Entry</div>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <input
                type="text"
                required
                placeholder="e.g. L'Oreal Paris Professional Shampoo 500ml"
                value={name}
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. L'Oreal, Schwarzkopf, Kerastase"
                value={brand}
                onChange={(e) => setBrand(e.target.value)}
                className="lss-input w-full text-xs font-semibold"
                list="add-product-brands"
              />
              <datalist id="add-product-brands">
                {uniqueBrands.map(b => (
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Luxury Salon Distributors"
                value={vendorName}
                onChange={(e) => setVendorName(e.target.value)}
                className="lss-input w-full text-xs font-semibold"
                list="add-product-vendors"
              />
              <datalist id="add-product-vendors">
                {uniqueVendors.map(v => (
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
            <span className="lss-overline text-amber-900">Pricing & Initial Stock</span>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            >
              ...
              <span>{submitting ? "Adding..." : "Add Product Item"}</span>
            </button>
          </div>
        </form>
      </motion.div>
