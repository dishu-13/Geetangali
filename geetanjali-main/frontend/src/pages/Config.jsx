...
export default function Config() {
  ...
  // Unmapped POS Products & Auto-Learned Custom Mappings State
  const [unmappedProducts, setUnmappedProducts] = useState([]);
  const [customMappings, setCustomMappings] = useState([]);
  const [unmappedSearch, setUnmappedSearch] = useState("");
  const [mapModal, setMapModal] = useState(null); // { pos_item_name, brand, pattern, amount } or null
  const [availableSkus, setAvailableSkus] = useState([]);

  const fetchUnmappedAndMappings = async () => {
    try {
      const [uRes, mRes] = await Promise.all([
        api.get("/incentives/unmapped-products"),
        api.get("/incentives/product-mappings"),
      ]);
      setUnmappedProducts(uRes.data?.unmapped || []);
      setCustomMappings(mRes.data || []);
    } catch (e) {
      console.error("Error fetching unmapped products/mappings", e);
    }
  };

  useEffect(() => {
    api.get("/config").then((r) => setCfg(r.data));
    fetchUnmappedAndMappings();
  ...
  // Custom Product Mapping Handlers
  const handleOpenMapModal = (item) => {
    setMapModal({
      pos_item_name: item ? item.item_name : "",
  ...
  const handleSaveProductMapping = async (e) => {
    e.preventDefault();
    if (!mapModal || !mapModal.pos_item_name) return;
    if (!mapModal.sku_id) {
      toast.error("Please select a target SKU from the master inventory list");
      return;
    }
    try {
      await api.post("/incentives/product-mappings", {
        pos_item_name: mapModal.pos_item_name,
        brand: mapModal.brand,
        pattern: mapModal.pattern,
        sku_id: mapModal.sku_id,
        amount: Number(mapModal.amount) || 0,
      });
      toast.success(`Mapping saved for '${mapModal.pos_item_name}'. Future POS sales will auto-assign ₹${mapModal.amount}!`);
      setMapModal(null);
      fetchUnmappedAndMappings();
    } catch (e) {
      toast.error(errMsg(e));
  ...
  const handleDeleteMapping = async (mappingId, displayName) => {
    if (window.confirm(`Delete auto-learned product mapping for '${displayName}'?`)) {
      try {
        await api.delete(`/incentives/product-mappings/${mappingId}`);
  ...
  return (
    <div className="p-6 sm:p-10 max-w-7xl mx-auto">
      ...
      {/* Flagged POS Products (Unmapped) & Auto-Learned Mappings */}
      <div className="lss-card p-6 mb-6 space-y-4 border-l-4 border-l-amber-500">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-amber-600 shrink-0" />
              <span className="lss-overline text-amber-900">POS Product Name Matching</span>
            </div>
            <h3 className="text-2xl font-extrabold text-slate-900 mt-1 flex items-center gap-2">
              Flagged POS Products 
              <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-200">
                {unmappedProducts.length} Needs Mapping
              </span>
            </h3>
            <p className="text-sm font-medium text-slate-600 mt-1">
              Products uploaded in POS that don't match any formula rule. Map them once to save a persistent rule for future POS uploads.
            </p>
          </div>
          <button
            onClick={() => handleOpenMapModal(null)}
            className="lss-btn-gold px-4 py-2 text-xs uppercase tracking-wider font-bold inline-flex items-center gap-1.5 shadow-sm shrink-0 self-start md:self-auto"
          >
            <Plus className="w-4 h-4" /> Add Custom Product Mapping
          </button>
        </div>
        ...
        {unmappedProducts.length > 0 && (
          <div className="relative w-full max-w-md pt-1">
            ...
            <input
              type="text"
              placeholder="Search unmapped POS product names..."
          ...
          <table className="lss-table text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-700">
                <th className="py-3 px-4 text-left">POS Product Name</th>
                <th className="py-3 px-4 text-left">Detected Brand</th>
                <th className="py-3 px-4 text-right">Sales Volume</th>
                <th className="py-3 px-4 text-right">Avg Net Price</th>
                <th className="py-3 px-4 text-left">Last Sold</th>
                <th className="py-3 px-4 text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {unmappedProducts
                .filter(u => !unmappedSearch || u.item_name.toLowerCase().includes(unmappedSearch.toLowerCase()))
                .map((u, i) => (
                  <tr key={i} className="hover:bg-amber-50/40 transition">
                    <td className="py-3 px-4 font-bold text-slate-900 flex items-center gap-2">
                      <Tag className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                      <span>{u.item_name}</span>
                    </td>
                    <td className="py-3 px-4 text-slate-600 font-semibold uppercase">
                      {u.brand || <span className="italic text-slate-400 font-normal">Unspecified</span>}
                    </td>
                    <td className="py-3 px-4 text-right tabular font-bold text-slate-800">
                      {u.sales_count} txns
                    </td>
                    <td className="py-3 px-4 text-right tabular font-extrabold text-emerald-800">
                      ₹{u.avg_net_price}
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        onClick={() => handleOpenMapModal(u)}
                        className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-bold text-xs inline-flex items-center gap-1 shadow-xs transition"
                      >
                        <Sparkles className="w-3.5 h-3.5" /> Map Rule Range
                      </button>
                    </td>
                  </tr>
                ))}
                ...
                <tr>
                  <td colSpan={6} className="py-8 text-center text-slate-600 font-medium">
                    <div className="flex flex-col items-center justify-center gap-1.5">
                      ...
                      <span className="font-bold text-slate-800 text-sm">All Uploaded POS Products Are Mapped!</span>
                      <span className="text-xs text-slate-500">Every retail product sold in POS matches an active incentive formula or saved alias rule.</span>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {/* Mapped POS Products Directory Reference */}
        <div className="pt-4 border-t border-slate-200">
          <h4 className="text-sm font-extrabold uppercase tracking-wider text-slate-800 mb-3 flex items-center gap-2">
            <Boxes className="w-4 h-4 text-amber-600" />
            Mapped POS Products Directory ({customMappings.length})
          </h4>
          <div className="overflow-x-auto border border-slate-200 rounded-xl bg-slate-50 max-h-72 overflow-y-auto">
            <table className="lss-table text-xs">
              <thead className="sticky top-0 bg-white z-10 shadow-xs">
                <tr className="bg-slate-50 text-slate-700">
                  <th className="py-2.5 px-4 text-left">POS Item Name</th>
                  <th className="py-2.5 px-4 text-left">Mapped SKU Product Name</th>
                  <th className="py-2.5 px-4 text-left">Brand</th>
                  <th className="py-2.5 px-4 text-left">Category</th>
                  <th className="py-2.5 px-4 text-right">Incentive Amount</th>
                  <th className="py-2.5 px-4 text-right">Action</th>
                </tr>
              </thead>
              <tbody>
                {customMappings.map((m) => {
                  const matchedSku = availableSkus.find((s) => s.id === m.sku_id);
                  return (
                    <tr key={m.id} className="hover:bg-amber-50/20 transition">
                      <td className="py-2 px-4 font-bold text-slate-900">{m.display_name}</td>
                      <td className="py-2 px-4 text-slate-700 font-semibold">
                        {matchedSku ? matchedSku.name : <span className="italic text-slate-400">Not selected</span>}
                      </td>
                      <td className="py-2 px-4 text-slate-600 font-semibold uppercase">{m.brand || matchedSku?.brand || "-"}</td>
                      <td className="py-2 px-4 text-slate-600 font-medium">{m.pattern || matchedSku?.category || "-"}</td>
                      <td className="py-2 px-4 text-right text-emerald-850 font-extrabold tabular">
                        ₹{m.amount}
                      </td>
                      <td className="py-2 px-4 text-right">
                        <button
                          onClick={() => handleDeleteMapping(m.id, m.display_name)}
                ...
                {customMappings.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-6 text-slate-500 font-medium">
                      No custom mappings configured. Use "+ Add Custom Product Mapping" or map flagged products above.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {mapModal !== null && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl max-w-md w-full p-6 text-slate-950 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-200 pb-4">
              <div>
                <span className="lss-overline text-amber-800 flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5 text-amber-600" /> Auto-Learning Rule Assignment
                </span>
                <h3 className="text-xl font-bold font-serif-lux text-slate-950 mt-0.5">
                  Map POS Product
                </h3>
            ...
            <form onSubmit={handleSaveProductMapping} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide block mb-1">
                  POS Product Name <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="Exact product name as printed in POS"
                  value={mapModal.pos_item_name}
                  onChange={(e) => setMapModal({ ...mapModal, pos_item_name: e.target.value })}
                  className="lss-input w-full text-xs font-bold text-slate-900 bg-slate-50"
                  list="pos-names-datalist"
                  required
                />
                <datalist id="pos-names-datalist">
                  {Array.from(new Set([
                    ...unmappedProducts.map(u => u.item_name),
                    ...uniqueSkuNames
                  ])).map(name => (
              ...
              <div className="overflow-visible">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide block mb-1">
                  Map to Master Inventory SKU <span className="text-rose-500">*</span>
                </label>
                <SearchableSelect
                  options={availableSkus}
                  value={mapModal.sku_id || ""}
                  onChange={(val, option) => {
                    if (option) {
                      setMapModal({
                        ...mapModal,
                        sku_id: val,
                        brand: option.brand || option.vendor_name || "",
                        pattern: option.category || "",
                      });
                    }
                  }}
                  placeholder="Search master inventory SKU..."
              ...
              {mapModal.sku_id && (
                <div className="grid grid-cols-2 gap-3 bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs animate-in fade-in duration-200">
                  <div>
                    <span className="text-slate-500 font-semibold block uppercase tracking-wider text-[10px]">Brand Reference</span>
                    <span className="text-slate-900 font-extrabold block mt-0.5">{mapModal.brand ? mapModal.brand.toUpperCase() : "Generic"}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 font-semibold block uppercase tracking-wider text-[10px]">Category Reference</span>
                    <span className="text-slate-900 font-extrabold block mt-0.5">{mapModal.pattern || "Uncategorized"}</span>
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  type="number"
                  placeholder="e.g. 150"
                  value={mapModal.amount}
                  onChange={(e) => setMapModal({ ...mapModal, amount: e.target.value })}
                  className="lss-input w-full text-xs font-bold text-emerald-800"
                  required
                />
                <p className="text-[11px] text-slate-500 mt-1">
                  This incentive rule will automatically be assigned whenever this product name appears in POS uploads.
                </p>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200">
                ...
                <button
                  type="submit"
                  className="lss-btn-gold px-5 py-2 text-xs font-bold uppercase tracking-wider shadow-xs inline-flex items-center gap-1.5"
                >
                  <Sparkles className="w-4 h-4" />
                  Save & Auto-Match Future Sales
                </button>
              </div>
            </form>
          </div>
